...
COPY --from=builder /app/target/x86_64-unknown-linux-musl/release/multipaint_by_numbers /multipaint_by_numbers-linux-amd64
COPY --from=builder /app/target/aarch64-unknown-linux-musl/release/multipaint_by_numbers /multipaint_by_numbers-linux-arm64

# Create arch-specific versions of image
FROM scratch AS runner
ARG TARGETOS
ARG TARGETARCH
COPY --from=binary /multipaint_by_numbers-${TARGETOS}-${TARGETARCH} /multipaint_by_numbers
ENTRYPOINT [ "/multipaint_by_numbers" ]
