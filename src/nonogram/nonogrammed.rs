...
use regex::Regex;
use rustls::{ClientConfig, client::WebPkiServerVerifier};

use super::{PopulatedBoard, populate_board};
...
pub async fn get_puzzle_data(id: u32) -> Result<NonogrammedPuzzle> {
    ...
    loop {
        ...
        if copyright.is_none()
            && let Some(caps) = USERNAME_RE.captures(line)
        {
        ...
        if solution.is_empty()
            && let Some(caps) = SOLUTION_RE.captures(line)
        {
            solution.extend(caps["solution"].chars().map(|char| char == '1'));
        }
        if rows.is_none()
            && let Some(caps) = ROWS_RE.captures(line)
        {
            rows = Some(
                caps["rows"]
                    .parse::<u16>()
        ...
        if columns.is_none()
            && let Some(caps) = COLUMNS_RE.captures(line)
        {
            columns = Some(
                caps["columns"]
                    .parse::<u16>()
